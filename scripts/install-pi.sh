#!/usr/bin/env bash
# Install Matinee on a Raspberry Pi running Tronbyt server.
# Can be invoked from anywhere — the script locates its own repo root.
set -euo pipefail

APP_DIR=/opt/matinee
DATA_DIR=/srv/matinee
CFG_DIR=/etc/matinee
...
# all of $HOME into /opt/matinee — not great).
...
systemctl stop matinee 2>/dev/null || true
...
install -m 0644 "$APP_DIR/scripts/matinee.service" /etc/systemd/system/matinee.service
systemctl daemon-reload
echo
echo "Installed. Next steps:"
echo "  1) Edit $CFG_DIR/config.toml (device_id, api_key, paths)."
echo "  2) Drop episodes under $DATA_DIR/sources/<Show>/*.mkv"
echo "  3) sudo -u pi $APP_DIR/venv/bin/matinee-ingest scan"
echo "  4) systemctl enable --now matinee"
